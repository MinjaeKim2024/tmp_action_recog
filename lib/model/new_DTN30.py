...
class RCMModule(nn.Module):
    def __init__(self, args, dim_head=64, method='New', merge='GAP'):
        ...
        self.avg_pool3d = nn.AvgPool3d(kernel_size=(1, 512, 1), stride=(1, 1, 1))
        self.avg_pool2d = nn.AvgPool2d(kernel_size=(512, 1), stride=(1, 1))

        # Self Attention Layers
        self.q = nn.Linear(30, dim_head * self.heads, bias=False)
        self.k = nn.Linear(30, dim_head * self.heads, bias=False)
        ...
        self.project = nn.Sequential(
            nn.Linear(30, hidden_dim, bias=False),
            nn.GELU(),
            nn.Linear(hidden_dim, 30, bias=False),
            nn.LayerNorm(30),
        )
        self.linear = nn.Linear(30, 512)
    ...
    def forward(self, x):
        ...
        xd_weight = self.project(self.avg_pool(inp.permute(0, 2, 1)).view(1, 30))
        xd_weight = torch.sigmoid(xd_weight).view(1, 30, 1)

        # Feature (X) direction
        q, k = self.q(x), self.k(x)
        q, k = map(lambda t: rearrange(t, 'b n (h d) -> b h n d', h=self.heads), [q, k])
        dots = einsum('b h i d, b h j d -> b h i j', q, k) * self.scale
        dots = self.avg_pool2d(dots)
        y = torch.sigmoid(dots).view(1, 512, 1) # problem line
    
        
        weight = einsum('b i d, b j d -> b i j', xd_weight, y)
        # weight = torch.zeros([1,30,512]).to(x.device)
        out = x * weight.permute(0, 2, 1)
        visweight = weight  # for visualization

        return out, self.linear(xd_weight.squeeze(0).squeeze(1)), visweight
...
class DTNNet(nn.Module):
    ...
    # ----------------------------------
    def f(self, n, sn):
        indices = torch.linspace(0, n-1, sn).long()  
        return indices
    ...
    def forward(self, img):  # img size: [2, 64, 1024]
        ...
        T = self._args.temper
        distillation_loss = F.kl_div(F.log_softmax(spatial_weights/ T, dim=-1),
        ...
        x = self.max_pool(torch.cat((xs.unsqueeze(2), xm.unsqueeze(2), xl.unsqueeze(2)), dim=-1))
        x = x.view(x.size(1), 1)
