...
if "Jester" in args.config:
    ...
    num_classes = 25
model = DSNNet(args, num_classes=num_classes, pretrained=args.pretrained, phase = args.phase)
...
cls_ = "nothing"
status = "nothing"
# ii_size = (3, 30, 224, 224)
# hh_size = (1, 30, 224, 224)
# from torchviz import make_dot

# ii = torch.randn(1, 3, 30, 224, 224).to(device)
# hh = torch.randn(1, 1, 30, 224, 224).to(device)
# make_dot(model(ii, hh)[0], params=dict(model.named_parameters()),show_attrs=True, show_saved=True).render("model", format="png")
# exit()
# from torchsummary import summary
# summary(model, input_size=[ii_size, hh_size], device=device.type)
with torch.no_grad():
    while True:
        frame_list = []
        idx = -1
