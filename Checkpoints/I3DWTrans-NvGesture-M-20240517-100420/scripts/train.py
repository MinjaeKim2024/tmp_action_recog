...
def main(local_rank, nprocs, args):
    ...
    for epoch in range(strat_epoch, args.epochs):
        ...
        wandb.log({'train_acc': train_acc, 'train_loss': train_obj, "learning_rate": optimizer.param_groups[0]['lr'], "valid_acc": valid_acc, "valid_loss": valid_obj})
        ...
        if local_rank == 0:
            ...
            state = {'model': model.state_dict(),'optimizer': optimizer.state_dict(), 'epoch': epoch + 1, 'bestacc': best_acc}
...
def train(train_queue, model, criterion, optimizer, epoch, local_rank):
    ...
    for step, (inputs, heatmap, target, _) in enumerate(train_queue):
        # target, target shape:  tensor([25]), torch.Size([1])
        ...
        # target: tensor([25], device='cuda:0') target shape: torch.Size([1])
        
        if args.MultiLoss:
            # shape of logits:  torch.Size([25]) shape of xs:  torch.Size([1, 25])
            # shape of xm:  torch.Size([1, 25]) shape of xl:  torch.Size([1, 25])
            
            if len(logits.size()) == 1:
                logits = logits.unsqueeze(0)
                # target = [25] -> [1,25]
            lamd1, lamd2, lamd3, lamd4 = map(float, args.loss_lamdb)
            # lamd1, lamd2, lamd3, lamd4:  1.0 0.5 0.5 0.5   
        ...
        if (step+1) % args.report_freq == 0 and local_rank == 0:
            log_info = {
                ...
                'Lr': [[round(float(g['lr']), 7),g['lr']] for g in optimizer.param_groups],
            }
